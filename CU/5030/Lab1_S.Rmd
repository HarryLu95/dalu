...
```{r}
# Faithful <- read.csv("Faithful.csv", head=TRUE)
Faithful <- read.csv("~/CORNELL/Courses/2018Fall/TA BTRY 4030 Linear Models with Matrices/Lab1/Faithful.csv")
```
...
where
  ...
  5. $\epsilon_i \sim N(0,\sigma^2_{\epsilon})$ for all $i = 1,\dots,n$ and are mutually independent
...
```
  ...
  c. Knit your document to see the output from the R code from (b).  Is the homogeneity of variance assumption met?  Is the normality assumption met? 
    ...
    homogeneity of variance: probably no
    
    normality assumption: probably yes
...
```
  ...
  length of eruption = -1.874016 + 0.075628 * time since last eruption
  
  5. Interpret the $R^2$ value of this model.
  
  Linear model fit explains 81.15% of the sample variance.
  
  6. Is there a significant linear relationship between expected `eruptions` and `waiting`?
  
  yes: t test p-value < 2e-16
  
  7. What is the value of $\hat\beta_1$?  Interpret $\hat\beta_1$ in the context of this analysis.
  
  0.075628
  
  for everytime 1 more unit of waiting, the next eruption lasts 0.075628 unit longer.
